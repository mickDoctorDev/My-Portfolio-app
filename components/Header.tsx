...
const Header: React.FC<HeaderProps> = ({ currentView, onNavigate, theme, toggleTheme }) => {
  ...
  const navLinks = [
    ...
    { id: 'timeline', label: 'Timeline', view: 'home' },
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <a
              ...
              className={`text-lg relative after:content-[''] after:absolute after:w-full after:h-0.5 after:bg-purple-400 after:bottom-0 after:left-0 after:transition-transform after:duration-300 hover:after:scale-x-100 hover:after:origin-bottom-left ${currentView === 'timeline' && link.id === 'timeline'
        ...
        <div className="flex flex-col items-center justify-center h-full space-y-8">
          {navLinks.map((link) => (
            <a
              key={link.id}
              href={`#${link.id}`}
              className={`text-3xl font-semibold hover:text-purple-500 dark:hover:text-purple-300 ${(currentView === 'timeline' && link.id === 'timeline') ? 'text-purple-600 dark:text-purple-400' : 'text-gray-800 dark:text-gray-200'
                }`}
              onClick={(e) => handleLinkClick(e, link.view as 'home' | 'timeline', link.id)}
            >
              {link.label}
